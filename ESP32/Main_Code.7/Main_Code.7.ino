...
void loop() {
  ...
  dribblerTurn(100);
  if(haveBall) haveBallLogic();
  else chaseBallUpField(); //chaseBallStraight();
...
void haveBallLogic(){
  ...
  if (goalX > 160) correctionSpeed = -map(goalX - 160, 0, 160, 0, 100);
  if (goalX < 160) correctionSpeed = map(160 - goalX, 0, 160, 0, 100);
  if (goalX > 150 && ballX < 170) correctionSpeed = 0;
