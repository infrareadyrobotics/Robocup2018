...
void drive(float Degrees, float targetHeading, int Speed, int rotateAmount) {
  ...
  if (Speed > 10) {
    ...
    if (0 > useHeading) {
      rotate = map(useHeading, -180, 0, maxRotateSpeed, 0);
      if (correctionDirection == -1) correctionTimer = millis();
      correctionDirection = 1;
    }
    else if (useHeading > 0) {
      rotate = map(useHeading, 0, 180, 0, maxRotateSpeed);
      if (correctionDirection == 1) correctionTimer = millis();
      correctionDirection = -1;
    }
    else rotate = 0;
    ...
    */
    if (rotateOnSpot == true) {
      if (useHeading > 5) rotate = -100;
      else if (useHeading < -5) rotate = 100;
      else {
        mSpeed[0] = 0;
        mSpeed[1] = 0;
        mSpeed[2] = 0;
...
//  Serial.print(ballY);
  ...
  */
  ...
  //    else drive(-130, 0 , cBSpeed * speedMultipler, 0);
  //  }
  //  else if (ballX == 0) {
  //    if (ballY > 150) drive(90, 0, cBSpeed * speedMultipler, 0);
  //    else drive(130, 0 , cBSpeed * speedMultipler, 0);
  //  }
  else if (ballX < 200 && ballX > 125) drive(0, 0, cBSpeed * speedMultipler, 0);
  else if (ballX > 200 && ballX + ballXRateOfChange < 200) drive(0, 0, cBSpeed * speedMultipler, 0);
  else if (ballX < 125 && ballX + ballXRateOfChange > 125) drive(0, 0, cBSpeed * speedMultipler, 0);
  //  else if (ballX > 160) drive(-90, 0, cBSpeed * speedMultipler, 0);
  //  else if (ballX < 160) drive(90, 0, cBSpeed * speedMultipler, 0);
  else if (ballX > 160) { // if on left and everything else is not true
...
void haveBallLogic() {
  ...
  if (ultrasonicData[0] < 20) {
    if (goalX > 160) drive(-90, 0, cBSpeed, 0);
    else drive(90, 0, cBSpeed, 0);
  }
  else if (goalX > 280 || goalX < 40) {
    drive(180, 0, cBSpeed, 0);
  }
  else if (goalX > 140 && goalX < 180) {
    ...
    correctionSpeed = -map(goalX - 160, 0, 160, 0, 60); //-map(goalX - 160, 0, 160, 0, 100);
    drive(0, 0, cBSpeed, correctionSpeed);
  }
  else if (goalX < 160) {
    //    Serial.println("CorrectRight");
    correctionSpeed = map(160 - goalX, 0, 160, 0, 60); //map(160 - goalX, 0, 160, 0, 100);
    drive(0, 0, cBSpeed, correctionSpeed);
  }
  else {
...
void haveBallCalc() {
  ...
  else if (haveBallTimer + 50 < millis()) {
    haveBall = false;
...
//  Serial.println();
  for (int i = 0;  i <= 6; i++) {
//    Serial.print(overWhiteLine[i]); //Error: 0, Black: 3, Green: 2, White: 0
//    Serial.print(',');
  }
  /*if (robotDirection == 90) {
    if (colourData[5] == 1 || colourData[6] == 1)
      else if (colourData[0] || colourData[3] || colourData[4]) drive(0, 0, 0, 0);
      else if (colourData)
  ...
  */
  ...
  //Colour Sensor 5 Detect if is outside whiteline
  if (lastColourData[5] == white && colourData[5] == green) {
    if ((headingCal > -45 && headingCal < 45) && colourData[6] == white) overWhiteLine[5] = true;
    else if (headingCal > 45 && headingCal < 135) {
      if (overWhiteLine[0] == true) {
        if (colourData[4] == white) overWhiteLine[5] = true;
      }
      else if (overWhiteLine[4] == true) {
        if (colourData[0] == white) overWhiteLine[5] = true;
      }
    }
    else if (headingCal < -45 && headingCal > -135) {
      if (overWhiteLine[0] == true) {
      ...
      overWhiteLine[5] = false;
    }
  }
  else if (colourData[5] == white) {
    overWhiteLine[5] = false;
  }

  /*
    //Colour Sensor 6 Detect if is outside whiteline
    if (lastColourData[6] == white && colourData[6] == green) {
      if ((headingCal > -45 && headingCal < 45) {
      if (overWhiteLine[5] == true) {
          if (colourData[0] == white || colourData[3] == white || colourData[4] == white) overWhiteLine[6] = true;
        }
        else if (overWhiteLine[0] == true || overWhiteLine[3] == true || overWhiteLine[4] == true) {
          if (colourData[5] == white) overWhiteLine[6] = true;
        }
      }
      else if ((headingCal > 45 && headingCal < 135) && colourData[4] == white) overWhiteLine[6] = true;
          else if ((headingCal < -45 && headingCal > -135) && colourData[0] == white) overWhiteLine[6] = true;
            else if ((headingCal < -135 || headingCal > 135) && colourData[5] == white) overWhiteLine[6] = true;
              else overWhiteLine[6] = false;
          }
    else if (colourData[6] == white) overWhiteLine[6] = false;
  */

  //Colour Sensor 0 Detect if is outside whiteline
  if (lastColourData[0] == white && colourData[0] == green) {
    if (headingCal > -45 && headingCal < 45) {
      if (overWhiteLine[5] == true || overWhiteLine[6] == true) {
        if (colourData[2] == white) overWhiteLine[0] = true;
      }
      else if (overWhiteLine[1] == true || overWhiteLine[2] == true) {
        if (colourData[6] == white) overWhiteLine[0] = true;
      }
    }
    else if ((headingCal > 45 && headingCal < 135) && (colourData[6] == white || colourData[5] == white || colourData[2] == white || colourData[1] == true)) overWhiteLine[0] = true;
    else if (headingCal < -135 || headingCal > 135) {
      if (overWhiteLine[5] == true || overWhiteLine[6] == true) {
        if (colourData[2] == white) overWhiteLine[0] = true;
      }
      else if (overWhiteLine[1] == true || overWhiteLine[2] == true) {
    ...
    else overWhiteLine[0] = false;
  }
  else if (colourData[0] == white) overWhiteLine[0] = false;



  /*
    if (colourData[5] == white)
    if (colourWasOnWhite[5] && colourData[6] == white) {
    colourWasOnWhite[6] = true;
    colurSensorOverWhiteLine[5] = true;
    Serial.println("ONRIGHTSIDEOFFIELD")
    }
    if (colourWasOnWhite[6] && (colorData[0] == 1 || colourData[3] == 1 || colourData[4] == 1)) {
    if (colourData[0] == 1) colourWasOnWhite[0] = true;
    if (colourData[3] == 1) colourWasOnWhite[3] = true;
    if (colourData[4] == 1) colourWasOnWhite[4] = true;
    colourSensorOverWhiteLine[6] = true;
    Serial.println("ONRIGHTSIDEOFFIELD");
    }*/
  for (int i = 0;  i <= 6; i++) {
    lastColourData[i] = colourData[i];
  }
}
