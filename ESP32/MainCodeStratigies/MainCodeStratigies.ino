...
void drive(float Degrees, float targetHeading, int Speed, int rotateAmount) {
  ...
  if (((useHeading < -5 || useHeading > 5) && rotateAmount == 0) && Speed > 60) Speed = 60;
...
void chaseBallUpField() {
  ...
  float speedMultipler = (float)map(ballY, 140, 170, 30, 100) / 100;
  if (ballY > 170) speedMultipler = 1;
  if (ballY < 140) speedMultipler = 0.3;

  if (ballX > 240 || ballX < 100) speedMultipler = 1;
  else {
    if (ballX > 170) speedMultipler += (float)map(ballX, 170, 200, 30, 100) / 100;
    else speedMultipler += (float)map(ballX, 170, 100, 30, 100) / 100;
    speedMultipler /= 2;
...
*/
  ...
  if (ballX == 320) { //to left out of vision
    if (ballY > 150) drive(-90, 0, cBSpeed, 0);
    else drive(-130, 0 ,cBSpeed,0);  
  }  
  else if (ballX == 0) {
    if (ballY > 150) drive(90, 0, cBSpeed, 0);
    else drive(130, 0 ,cBSpeed,0);  
  }
  else if(ballX < 200 && ballX > 125) drive(0,0,0,0);
  else if (ballX > 200 && ballX + ballXRateOfChange < 200) drive(0, 0, 0, 0);
  else if (ballX < 125 && ballX + ballXRateOfChange > 125) drive(0, 0, 0, 0);
  else if (ballX > 160) drive(-90, 0, cBSpeed, 0);
  else if (ballX < 160) drive(90, 0, cBSpeed, 0);*/
...
void laserRange() {
  ultrasonicData[0] = lidar.readRangeSingleMillimeters() / 10;
}

void onWhiteFunc(){

  for(int i = 0;  i <= 6; i++){ 
  Serial.print(colorData[i]); //Error: 0, Black: 3, Green: 2, White: 0
  Serial.print(',');
  }
