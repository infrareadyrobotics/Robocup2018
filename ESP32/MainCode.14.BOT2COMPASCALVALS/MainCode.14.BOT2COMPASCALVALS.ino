...
unsigned long kickdribblerofftimer;
int hBSpeed = 90;
...
void haveBallLogic() {
  ...
  /*
    ...
    if (goalHeight > 20) {
      ...
      dribblerTurn(0);
      if (kickdribblerofftimer + 100 < millis()) {
        if (motorsOn) kick();
      }
    }
  }
    ...
    correctionSpeed = -map(goalX - 160, 0, 160, 0, 30); //-map(goalX - 160, 0, 160, 0, 100);
    drive(0, 0, hBSpeed, correctionSpeed);
  }
  else if (goalX < 160) {
    //    Serial.println("CorrectRight");
    correctionSpeed = map(160 - goalX, 0, 160, 0, 30); //map(160 - goalX, 0, 160, 0, 100);
    drive(0, 0, hBSpeed, correctionSpeed);
  }
  else {
