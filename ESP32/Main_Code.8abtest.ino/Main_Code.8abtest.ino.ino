...
void loop() {
 static int iii = 0;
  ...
  if (blocks)
  {
    ...
    Serial.print(buf);
    for (jj = blocks - 1; jj >= 0; jj--)
    {
      sprintf(buf, "  block %d: ", jj);
      Serial.print(buf);
      Serial.print("pixy.blocks[jj].x: ");
      Serial.print(pixy.blocks[jj].x);
      Serial.print("\tpixy.blocks[jj].y: ");
      Serial.print(pixy.blocks[jj].y);
      Serial.print("\tpixy.blocks[jj].width: ");
      Serial.print(pixy.blocks[jj].width);
      Serial.print("\tpixy.blocks[jj].height: ");
      Serial.print(pixy.blocks[jj].height);
      Serial.print("\tpixy.blocks[jj].signature: " );
      Serial.println(pixy.blocks[jj].signature);
    }
  }
