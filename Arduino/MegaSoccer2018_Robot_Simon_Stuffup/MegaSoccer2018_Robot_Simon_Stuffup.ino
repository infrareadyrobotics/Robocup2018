...
void loop() {
  ...
  ballPos = (float)ballY - (float)ballHeight / 2.0;
  ballDist = distC * tan(float(ballPos) / distA) + distB;

  if (ballDist <= 75 && canSeeBall) bypassState = attack;
  //else bypassState = robotType;
  if (goalColour == blue) {
    Serial.print("Blue");
    goalY = bgoalY;
    goalX = bgoalX;
  ...
  goalPos = goalY - goalHeight / 2;
  goalDist = distC * tan(goalPos / distA) + distB;
  avgGoalDist(goalDist);

//  Serial.print("ballDist:\t");
//
//  Serial.print(ballDist, 2 );
//  Serial.print("LastBallDist:\t");
//
//  Serial.print(lastBallDist, 2);
//  Serial.print("\t");
  if (ballDist != lastBallDist) {
    pixyTimer[3] = 0;
    canSeeBall = true;
  }
  if (pixyTimer[3] >= 10 && ballDist >= 2.5) {
    canSeeBall = false;
    Serial.print("Error Ball missing,\t");

  }
  else if (ballDist == lastBallDist) {
    pixyTimer[3]++;
  ...
  if (ballDist >= 250) {
    ballDist = 250;
    Serial.print("Error Ball imposibbly far away,\t");
  }
//  Serial.print(pixyTimer[3]);
//  Serial.print("ballDist:\t");
//  Serial.print(ballDist, 5);
  if (goalDist >= 250) {
    goalDist = 250;
    Serial.print("Error Goal imposibbly far away,\t");
  }
//  Serial.print("goalDist:\t");
//  Serial.print(goalDist, 2);
  pixyCam();
//  Serial.println("test");
  disableMotorsButton();
...
//
correctAmount = 100 - correctAmount;
  if (correctAmount < 60) {
...
void Chaseball() {
  ...
  if (bypassState == attack) Serial.print("Attack,\t");
  if (bypassState == defend) Serial.print("Defend,\t");
  //1 = Forward
  //2 = Forward Left
  //3 = Forward Right
  //4 = Back Left
  ...
  Serial.print(goalDist);
  Serial.print(",\t");
  Serial.print(goalLocation - 15);
  Serial.print(",\t");
  Serial.print((defend == bypassState || (defend == bypassState && !canSeeBall)));// //
  Serial.print(",\t");
  if (goalDist <= goalLocation - 15 && (defend == bypassState || (defend == bypassState && !canSeeBall))) {// && canSeeBall
    if (goalX < 165 && goalX > 155) Direction = 6;
    else if (goalX > 160) Direction = 4;
    else if (goalX < 160) Direction = 5;
  }
  else if ((goalDist >= goalLocation - 15 && goalDist <= goalLocation + 15) && (defend == bypassState || (defend == bypassState && !canSeeBall))) Direction = 0;
  else if (goalDist >= goalLocation + 20 && (defend == bypassState || (defend == bypassState && !canSeeBall))) Direction = 1;
  else if (ballX < 240 && ballX > 80) {//
    Direction = 1;
    ...
    Direction = 6;
    if (defend == bypassState) goToGoal();
  }
    ...
    if (ballY >  60) { // makes sure the ball is not too close
      ...
      if (defend == bypassState && goalDist <= goalLocation) Direction = 4;
      else if (defend == bypassState && goalDist >= goalLocation - 15 && goalDist <= goalLocation + 15) Direction = 0;
    }
    else {
    ...
    if (ballY > 60) {
      ...
      if (defend == bypassState && goalDist <= goalLocation) Direction = 5;
      else if (defend == bypassState && goalDist >= goalLocation - 15 && goalDist <= goalLocation + 15) Direction = 0;
    }
    else {
  ...
  if (bypassState == defend) Direction = 0;



  if (Direction == 0) Serial.print("Stoped");
  else if (Direction == 1) Serial.print("Forwards");
  else if (Direction == 2) Serial.print("Forwards Left");
  else if (Direction == 3) Serial.print("Forwards Right");
  else if (Direction == 4) Serial.print("Back Left");
  else if (Direction == 5) Serial.print("Back Right");
  else if (Direction == 6) Serial.print("Back");
}

void goToGoal() {
  Serial.print("GoToGoal,\t");
  if (goalDist <= goalLocation + 15) Direction = 6;
  else Direction = 0;
}
...
void haveBallCalc() {
  ...
  Serial.print("\tballX,\t");
  Serial.print(ballX);
  if (ballY <= 25 && ballX < 205 && ballX > 115 && ballDist < 5) {
...
void CorrectDirection () {
  ...
  //Serial.print(buf);
  Serial.print(bypassState == attack);
  Serial.print(!(0 == goalX || 320 == goalX));
  Serial.print(0 == goalX);
  Serial.print(320 == goalX);

  if (!(0 == goalX || 320 == goalX) && bypassState == attack && Direction != 6 && canSeeBall) {//
    Serial.print("FacingGoal");
    correctAmount = abs(160 - goalX) * correctAmountMultiplier;
    ...
    Serial.print(calibratedCompassValue);
    correctAmountMultiplier = 1;
    correctAmount = byte(abs(calibratedCompassValue) * correctAmountMultiplier);
...
void avgGoalDist(float newVal) {
  char output[50];
  sprintf(output, "Size of goalDist: %d", sizeof(goalDistAvg));
  //  Serial.println(output);
  for (int i = sizeof(goalDistAvg) / 4 - 1; i > 0 / 4; i--) {
    goalDistAvg[i] = goalDistAvg[i - 1];
    sprintf(output, "Val %d is %d", i + 1, (int)goalDistAvg[i]);
    //    Serial.println(output);
  }
  goalDistAvg[0] = newVal;
  sprintf(output, "Val %d is %d", 1, (int)goalDistAvg[0]);
  //  Serial.println(output);
  for (int i = 0; i < sizeof(goalDistAvg) / 4; i++) {
    goalDist += goalDistAvg[i];
  }
  goalDist /= sizeof(goalDistAvg) / 4;
  sprintf(output, "GoalDist: %d", (int)goalDist);
  //  Serial.println(output);
}
