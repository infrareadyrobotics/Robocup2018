...
void loop()
{
  ...
  if (DEBUG) {
    Serial.print("\nSwitch 1: ");
    Serial.print(digitalRead(EchoPIN[0]));
    Serial.print("\nSwitch 2: ");
    Serial.print(digitalRead(TriggerPIN[0]));
    Serial.println();
  }
}
...
void i2c()
{
  ...
  Wire.write(digitalRead(EchoPIN[0]));
  Wire.write(digitalRead(TriggerPIN[0]));
...
void detectColour()
{
  if (ColourIN[0] >= 0 && ColourIN[0] < 500)
    { //White
      colourValue[0] = 1;
    }
    else if (ColourIN[0] >= 1000 && ColourIN[0] <= 3650)
    { //Green
      colourValue[0] = 2;
    }
    else if (ColourIN[0] >= 3650 && ColourIN[0] <= 4500)
    { //Black
      colourValue[0] = 3;
    }
    else if (ColourIN[0] >= 5501)
    { //Error
      colourValue[0] = 0;
    }
  for (int i = 1; i <= 6; i++)
  {
    ...
    else if (ColourIN[i] >= 500 && ColourIN[i] <= 1500)
    { //Green
      colourValue[i] = 2;
    }
    else if (ColourIN[i] >= 1501)
    { //Error
      colourValue[i] = 0;
    }
  }
