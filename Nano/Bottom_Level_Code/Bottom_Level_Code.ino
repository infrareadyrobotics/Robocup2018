...
void debugColourSensors() {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    else if (onGreen[i] == HIGH) {
      Serial.print("\tand is Green");
    }
    else if (onBlack[i] == HIGH) {
      Serial.print("\tand is Black");
    }
    else if (colorSensorError[i] == HIGH) {
...
void detectColour() {
  for (int i = 0; i < 6; i++) {
    if (ColourIN[i] >= 0 && ColourIN[i] < 200 && onWhite[i] != HIGH) { //White
      colorSensorError[i] = LOW;
      onWhite[i] = HIGH;
      onGreen[i] = LOW;
      onBlack[i] = LOW;
    }
    else if (ColourIN[i] >= 500 && ColourIN[i] <= 1000 && onGreen[i] != HIGH) { //Green
      colorSensorError[i] = LOW;
      onWhite[i] = LOW;
      onGreen[i] = HIGH;
      onBlack[i] = LOW;
    }
    else if (ColourIN[i] >= 1001 && ColourIN[i] <= 2000 && onBlack[i] != HIGH) { //Black
      colorSensorError[i] = LOW;
      onWhite[i] = LOW;
      onGreen[i] = LOW;
      onBlack[i] = HIGH;
    }
    else if (ColourIN[i] >= 2001) { //Error
      onWhite[i] = LOW;
      onGreen[i] = LOW;
      onBlack[i] = LOW;
      colorSensorError[i] = HIGH;
    }
  }
}
