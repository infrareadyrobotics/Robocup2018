...
void i2c()
{
  ...
  Wire.write(switchState[0]);
  Wire.write(switchState[1]);

  Serial.println(ballX);
  Serial.println(ballY);
  Serial.println(ballWidth);
  Serial.println(ballHeight);
  ...
  Serial.println(goalWidth);
  Serial.println(goalHeight);

  Wire.write(goalX);
  Wire.write(goalY);
  Wire.write(goalWidth);
  Wire.write(goalHeight);
}
...
void pixyCam() {
  ...
  if (blocks) {
    ...
    for (int q = blocks - 1; q >= 0; q--) {
      ...
      else if (pixySigniture == 2)
      {
        //        Serial.print("PixyBlueGoal\n");
        pixyTimer[1] = 0;
        bgoalX = pixyX;
        bgoalY = pixyY;
      ...
      else if (pixySigniture == 3)
      {
        //        Serial.print("PixyYellowGoal\n");
        pixyTimer[2] = 0;
        ygoalX = pixyX;
        ygoalY = pixyY;
  ...
  if (goalColour == blue) {
    ...
    goalWidth = ygoalWidth;
  }
}

void switches() {
  switchState[0] = digitalRead(EchoPIN[0]);
  switchState[1] = digitalRead(TriggerPIN[0]);
